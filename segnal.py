...
def gelfgat_deconvolve2d(F, q, where=None, illegal=None, verbose=False, show_plots=False):
	...
	while len(scores) < 200:# and (iterations < 6 or χ2_red > χ2_red_95):
		...
		if show_plots:
			...
			# axes[0].set_title("Fit source image")
			axes[0].set_title(f"Iteration {len(scores)}")
			plot = axes[0].pcolormesh(N*g/η, vmin=0, vmax=N*(g/η).max(), cmap=GREYS)
			axes[0].axis('square')
			axes[0].set_xticks([])
			axes[0].set_yticks([])
			# fig.colorbar(plot, ax=axes[0])
			...
			# axes[1].set_title("Fit penumbral image")
			plot = axes[1].pcolormesh(N*s.T, vmin=0, vmax=F.max(where=where, initial=0), cmap='viridis')
			axes[1].axis('square')
			axes[1].set_xticks([])
			axes[1].set_yticks([])
