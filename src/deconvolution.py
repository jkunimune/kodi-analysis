...
def gelfgat(F: NDArray[float], q: NDArray[float],
	...
	for t in range(num_iterations):
		...
		if show_plots and t%20 == 0:
			...
			axes[0, 0].imshow(N*(g/η).T, vmin=0, vmax=N*(g/η).max(), cmap=CMAP["greys"], origin="lower")
			axes[0, 1].set_title("Floor")
			axes[0, 1].imshow(g.T, norm=SymLogNorm(vmin=0, linthresh=np.min(g, where=g > 0, initial=inf)*6, linscale=1/log(10)), cmap=CMAP["greys"], origin="lower")
